...
<div {{ $attributes->merge(['class' => 'grid grid-cols-1 gap-2 sm:flex sm:flex-wrap']) }}>
    <button type="button" onclick="shareWhatsApp('{{ $url }}', '{{ addslashes($title) }}')"
            class="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-green-600 px-4 py-3 text-sm font-medium text-white hover:bg-green-700 sm:w-auto sm:py-2">
    ...
    <button type="button" onclick="copyToClipboard('{{ $url }}')"
            class="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-stone-100 px-4 py-3 text-sm font-medium text-stone-700 hover:bg-stone-200 sm:w-auto sm:py-2">
    ...
    <a href="https://twitter.com/intent/tweet?url={{ urlencode($url) }}&text={{ urlencode($title) }}"
       ...
       class="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-stone-900 px-4 py-3 text-sm font-medium text-white hover:bg-stone-800 sm:w-auto sm:py-2">
        Compartilhar
    </a>
</div>
